...
# Update — 2026-09-13: the branch is being split into four stacked PRs

`feature/host-api-actions` (tip `c667c10`, 52 commits, hardware-regression-tested) is being split on
Jeroen's instruction into four **stacked** PRs, each collapsed to net effect. Stacked rather than
independent because `config.lua` and `Tests/lua/harness.lua` are touched by all three code themes, often
in the same functions — independent PRs would conflict badly and none would contain the tested tree.

**`feature/host-api-actions` must not be modified.** It is the reference tree until the split is done.

## The stack, and where it stands

| # | Branch | Base | State |
|:--|:--|:--|:--|
| 1 | `split/1-control-declarations` | `main` | **Done** — `187cbd8`, 101/101 green |
| 2 | `split/2-session-identification` | (1) | **Done** — `3b23999`, 185/185 green |
| 3 | `split/3-master-volume` | (2) | **Not started** |
| 4 | `split/4-release-docs` | (3) | **Not started** |

Nothing is pushed. No PRs opened.

## PR3 — Master Volume + probe + its tests

Everything Master Volume, verified cleanly separable during PR2 and deliberately left out of it:

- `IT_MASTER_VOLUME`, `MVOL_READ`/`MVOL_WRITE`, `msg_master_volume_write`/`_read`, `masterVolume`,
  `masterVolumeRead`, `MVOL_SEED_DEFAULT` (60).
- `mvolFlushReady` and `flush_pending`'s `is_paced_and_blocked` — the one-write-per-tick pacing.
- `draw_popup_erase`, `enter_popup_mode`, `POPUP_ERASE_OVERLAP_IDS`, `popupMax`, and
  `POPUP_DISMISS_IDLE_TICKS` 1→2 (introduced by `a99db47`, confirmed via `git log -S`, not session work).
- `enter_active_session()`'s `queue_master_volume_read()` call and `handle_login`'s MV resync. PR2 kept
  the session-transition idempotency but stripped the MV read — PR3 adds it back.
- `Scripts/probe-mastervolume.swift` in full, including `--cadence` mode.
- Harness tests 27-28, 30, 32, 35, 37-39, 43-48, 52-54 *by the original `c667c10` numbering*.

## PR4 — release, docs, agent guidance

`CHANGELOG.md`, `CLAUDE.md`, `AGENTS.md`, `.claude/skills/**`, `.github/workflows/release.yml`,
`Scripts/restart-mainstage.sh`, `Scripts/install-mainstage-script.sh`, and all of `docs/`.

**PR4 owns the version decision, deliberately.** No earlier PR carries a `BREAKING CHANGE:` trailer, so
merging PR1-PR3 cannot trigger a release early. PR4 must add a real `BREAKING CHANGE:` trailer describing
the relative-encoder switch — note the `feat!: rename the device from SL to SL88` commit that currently
forces `major` was itself reverted and does **not** appear in any split branch, so without the trailer the
workflow would choose the wrong bump.

## The check that must hold at the end

```
git diff split/4-release-docs feature/host-api-actions
```

**must be empty.** If it is not, the split lost or duplicated something, and no amount of clean-looking
per-PR diffs makes up for it. Run this before opening any PR.

Current remaining delta after PR2: 3,986 insertions across 18 files — all of it PR3 and PR4.
