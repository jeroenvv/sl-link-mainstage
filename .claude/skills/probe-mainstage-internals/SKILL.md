...
`controller_info()` is parsed by `LogicPro.framework`, not by MainStage's own binary — MainStage and
Logic Pro share the control-surface code:

```
/Applications/MainStage 3.7.1.app/Contents/Frameworks/LogicPro.framework/Versions/A/LogicPro
```
...
```bash
LP="/Applications/MainStage 3.7.1.app/Contents/Frameworks/LogicPro.framework/Versions/A/LogicPro"
strings -a -t d "$LP" | grep -n "LUA: controller_info"
strings -a -t d "$LP" | awk '$1>=10987500 && $1<=10989400 {print}'
```

Offsets shift between MainStage versions — re-locate via the `LUA: controller_info` anchor rather
than reusing the numbers above. The table appears twice in the binary.
...
```bash
plutil -p "/Applications/MainStage 3.7.1.app/Contents/Resources/en.lproj/WsCommands.plist"
```
...
```bash
MS="/Applications/MainStage 3.7.1.app/Contents/Frameworks/MACore.framework/Versions/A/Resources/MIDI Device Scripts"
...
```bash
grep -rl --binary-files=text "<token>" "/Applications/MainStage 3.7.1.app" 2>/dev/null
```

A hit in `MainStageCore` or `LogicPro` means the string exists somewhere in the app; no hit anywhere
means it is very likely inert.
...
3.7.1. It was tested across six configurations (script-injected CCs on channel 16 and channel 1, real
hardware CC 1 and CC 16, both the `action_<app>` and bare `action` spellings, with and without
`objectType`) and never fired. The key table lives in *LogicPro*.framework, so a key found there may
be a Logic Pro feature MainStage simply does not implement.
